...
$(function () {
    var Datepicker = function (div, y, m, d) {
        ...
        this.createTableNum = function () {
            ...
            this.table = table;
            this.td = this.table.find('tr:not(.date-plus-tr):not(.info-title-tr):not(.week-tr)').children('td');
        ...
        this.dateScale = function (y, m, d) {
            ...
            for (var i = 0; i < dayLength; i++) {
                this.td.eq(i + fd).attr('id', 'd-' + y + '-' + (m + 1) + '-' + (i + 1));
    ...
    Datepicker.prototype.initial = function () {
        ...
        while (i--) this.setKeyuyue(this.PArr[i].date, this.PArr[i].p);
    }
    // 指定可预约日期
    Datepicker.prototype.setKeyuyue = function (date, p) {
        ...
        _td.on('click', function () {
            ...
            if (p.s) {
                cloneTr.find('.s').prop('checked', ''/*p.s*/);
            } else {
                cloneTr.find('.s').replaceWith('<span class="yibeiyuyue-span">已被预约</span>');
            }
            if (p.z) {
                cloneTr.find('.z').prop('checked', ''/*p.z*/);
            } else {
                cloneTr.find('.z').replaceWith('<span class="yibeiyuyue-span">已被预约</span>');
            }
            if (p.x) {
                cloneTr.find('.x').prop('checked', ''/*p.x*/);
            } else {
                cloneTr.find('.x').replaceWith('<span class="yibeiyuyue-span">已被预约</span>');
            }
            ...
            cloneTr.find('.confirm-anchor').on('click', function () {
                ...
                var returnValue = date[0] + ', ' + date[1] + ', ' + date[2] + ', ' + s.prop('checked') + ', ' + z.prop('checked') + ', ' + x.prop('checked');
                //alert(returnValue);

                if (s.prop('checked') || z.prop('checked') || x.prop('checked')) {
                    var str = '';
                    if (s.prop('checked')) {
                        str = '<span class="orange">上午</span>，';
                    }
                    if (z.prop('checked')) {
                        str += '<span class="orange">中午</span>，';
                    }
                    if (x.prop('checked')) {
                        str += '<span class="orange">下午</span>';
                    }
                    ...
                    _this.info_ul.append('<li><p class="oid-' + date.join('-') + '">您已成功预约：' + date[0] + '年' + date[1] + '月' + date[2] + '日 ' + str + '</p></li>');
                    //intoTempArr.push(new specialDate(date.join('-'), {s: str[0], z: str[1], x: str[2]}));

                    /*
                     var dp = new Datepicker($('.a'), 2015, 5, 16);
                     dp.PArr.push(new specialDate('2015-5-17', { 's': false, 'z': false, 'x': false }));
                     dp.initial();
                     */
                }
    ...
    // 首次请求
    var dp = new Datepicker($('.a'), 2015, 5, 15);
    dp.PArr.push(new specialDate('2015-5-16', { 's': false, 'z': false, 'x': false }));
    dp.PArr.push(new specialDate('2015-5-17', { 's': false, 'z': true, 'x': false }));
    dp.PArr.push(new specialDate('2015-5-18', { 's': false, 'z': false, 'x': false }));
    dp.PArr.push(new specialDate('2015-5-19', { 's': false, 'z': true, 'x': true }));
    dp.PArr.push(new specialDate('2015-5-20', { 's': false, 'z': true, 'x': true }));
    dp.PArr.push(new specialDate('2015-5-21', { 's': true, 'z': true, 'x': true }));
    dp.PArr.push(new specialDate('2015-5-22', { 's': false, 'z': true, 'x': false }));
    dp.PArr.push(new specialDate('2015-5-23', { 's': false, 'z': true, 'x': false }));
    dp.PArr.push(new specialDate('2015-5-24', { 's': false, 'z': true, 'x': false }));
    dp.PArr.push(new specialDate('2015-6-16', { 's': false, 'z': true, 'x': false }));
    dp.PArr.push(new specialDate('2015-7-17', { 's': false, 'z': true, 'x': false }));
    dp.initial();
